...
const NewPaymentMethod = ({ toggle }) => {
  return (
    <div
      className={`${
        toggle ? "h-670" : "h-0"
      } overflow-hidden transition-custom-all duration-custom-1000 custom-ease`}
    >
      <hr className="w-8/12 m-auto my-3 border-t-2 border-gray-400" />
      <h2 className="text-sm text-center mb-1 md:text-left">
        "Name Ecommerce" accepts this credit and debit cards
      </h2>
      ...
      <form>
        ...
        <fieldset>
          ...
          <div>
            <label htmlFor="phoneNumber" className="block text-xs mb-1">
              Phone number
            </label>
