...
const LateralMenu = ({ isOpen }) => {
  const openMenu = `${
    isOpen ? " right-0 xl-h-500" : "-right-full xl:h-0 xl:p-0"
  }`;
  return (
    <aside
      className={`${openMenu} hidden md:flex flex-col transition-custom-all duration-custom-1000 ease-custom p-6 w-80 bg-gray-300 fixed top-20 bottom-0 z-50 xl:top-16 xl:bottom-auto xl:right-0 xl:overflow-hidden`}
    >
